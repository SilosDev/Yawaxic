...
iData.value10.SHAPE_FILE = "kw_1357PlaceShape.txt"
...
handler(function()
	...
	while iData.value17() do
		...
		if iData.value14.AutoSteal and iData.value23() then
			...
			if not success then
				warn("[kw_1357] steal: " .. tostring(secondaryResult))
				task.wait(0.5)
			end
		else
...
function iData.value90()
	...
	iData.value11.espFolder.Name = "kw_1357EggESP"
...
iData.value10.HOP_FILE = "kw_1357HopCount.txt"
...
handler(function()
	while iData.value17() do
		...
		if iData.value119.AutoSteal and iData.value23() and (iData.value132() and not iData.value126) then
			...
			if not success then
				warn("[kw_1357Free] " .. tostring(result))
			end
...
function iData.value138()
	...
	ScreenGui.Name = "kw_1357StealFreeGui"
	ScreenGui.ResetOnSpawn = false
	ScreenGui.IgnoreGuiInset = true
...
local function createServerHopGui()
	...
	TitleLabel.Text = "kw_1357 Hop Sever"
...
local function createStealGui()
	...
	local ScreenGui = Instance.new("ScreenGui")
	ScreenGui.Name = "kw_1357RecallStealGui"
	ScreenGui.ResetOnSpawn = false
	ScreenGui.IgnoreGuiInset = true
...
iData.value12.TabConfig:Button({
	...
	Callback = function()
		...
		if writefile then
			pcall(writefile, "kw_1357Remotes.txt", concatResult)
		end
		...
		task.spawn(function()
			...
			repeat
				if copyPlotDumpFlag or not nameOption then
					...
					if writefile then
						pcall(writefile, "kw_1357Plot.txt", concatResult)
					end
					iData.value18("Plot Dump", "Copied. Also saved as kw_1357Plot.txt.", 6)
	...
	Callback = function()
		task.spawn(function()
					...
					if descendant:IsA("ModuleScript") or descendant:IsA("LocalScript") then
						...
						if lower:find("egg") or (lower:find("plac") or (lower:find("build") or lower:find("plot"))) then
							...
							then
								...
								pcall(writefile, "kw_1357Place_" .. descendant.Name .. ".txt", copyPlaceSourceResult)
			...
			if secondaryDescendant and copyPlaceSourceFlag then
				...
				if success then
					pcall(writefile, "kw_1357EggState.txt", copyPlaceSourceResult)
			...
			if writefile then
				pcall(writefile, "kw_1357PlaceSrc.txt", concatResult)
			end

			iData.value18("Place Source", "Copied. Saved kw_1357PlaceSrc.txt + per-module files.", 6)
		end)
	end,
})
iData.value12.TabConfig:Button({
...
iData.value143.Name = "kw_1357VisualPets"
...
if not iData.value24 then
	iData.value18("kw_1357Hub", "Packages.Networking is missing, remote features are offline.", 8)
	return
end
iData.value18("kw_1357Hub", "Loaded. " .. #iData.value13.areaOrder .. " areas, " .. #iData.value28() .. " endpoints.", 5)
